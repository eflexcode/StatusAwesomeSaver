...
public class WhatsappVideoRepository {
    ...
    public WhatsappVideoRepository(Context context) {
        this.context = context;
    }


    public void getVideos(final Handler handler) {

        if (Constance.WhatsAppStatusesDirectory.exists()) {

            new Thread(new Runnable() {
                @Override
                public void run() {
                    File[] files = Constance.WhatsAppStatusesDirectory.listFiles();
    ...
    private Bitmap getThumb(StatusModel statusModel) {
        ...
        if (statusModel.isVideo()) {
            return ThumbnailUtils.createVideoThumbnail(statusModel.getFile().getAbsolutePath(), MediaStore.Video.Thumbnails.MINI_KIND);
        } else {
